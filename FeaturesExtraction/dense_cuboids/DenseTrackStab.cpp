...
int main(int argc, char** argv)
{
  ...
  while(true)
  {
    ...
    for(int iScale = 0; iScale < scale_num; iScale++)
    {
      ...
      // We don't track features anymore, only save their descs
      std::list<Track>& tracks = xyScaleTracks[iScale];
      for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();)
      {
        ...
        if(iTrack->index >= trackInfo.length)
        {
          ...
          //float mean_x(0), mean_y(0)), var_y(0), length(0);
          // We save all cuboids,
          //if(IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement))
          {
            // Each trajectory have size one
            // mean_x = trajectory[0].x;
            // mean_y = trajectory[0].y;


            // output the trajectory
            // printf("%d\t%f\t%f\t%f\t", frame_num, mean_x, mean_y, fscales[iScale]);
            ...
            PrintDescCuboids(iTrack->hog, hogInfo, trackInfo);
            PrintDescCuboids(iTrack->hof, hofInfo, trackInfo);
            PrintDescCuboids(iTrack->mbhX, mbhInfo, trackInfo);
            PrintDescCuboids(iTrack->mbhY, mbhInfo, trackInfo);
            printf("\n");
          }
          iTrack = tracks.erase(iTrack);
          continue;
