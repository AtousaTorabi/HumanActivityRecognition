...
void DenseSampleCuboids(const Mat& grey, std::vector<Point2f>& points, const int cuboids_stride)
{
  int width = grey.cols / cuboids_stride;
  int height = grey.rows / cuboids_stride;

  std::vector<int> counters(width*height);
  int x_max = cuboids_stride * width;
  int y_max = cuboids_stride * height;
  for(int i = 0; i < points.size(); i++)
  {
    Point2f point = points[i];
    int x = cvFloor(point.x);
  ...
  for(int i = 0; i < height; i++)
    for(int j = 0; j < width; j++, index++)
    {
      ...
      int x = j * cuboids_stride + offset;
      int y = i * cuboids_stride + offset;
...
void PrintDescCuboids(std::vector<float>& desc, DescInfo& descInfo, TrackInfo& trackInfo)
{
  int tStride = cvFloor(trackInfo.length/descInfo.ntCells);
  float norm = 1./float(tStride);
  int dim = descInfo.dim;
