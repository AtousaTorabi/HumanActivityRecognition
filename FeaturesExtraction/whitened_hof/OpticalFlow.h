...
PyObject* pylearn2_datasets_processing;
PyObject* zca_obj;
PyObject* fit_name;
PyObject* display_fnp;
PyObject* display_do_fnp;
PyObject* numpy_dot_fnp;
PyObject* numpy_sub_fnp;

void calcOpticalFlowFarneback(std::vector<Mat>& prev_poly_exp_pyr, std::vector<Mat>& poly_exp_pyr,
                              std::vector<Mat>& flow_pyr, int winsize, int iterations, bool warped=false)
{
    int i, k;
    Mat prevFlow, flow;
    ...
    for( k = flow_pyr.size() - 1; k >= 0; k-- )
    {
        ...
        if(warped){
            ...
            if(my::first_time){
                my::first_time = 0;
                PyObject* module = PyString_FromString("pylearn2.datasets.preprocessing");
                pylearn2_datasets_processing = PyImport_Import(module);
                Py_DECREF(module);

                PyObject* zca_fnp = PyObject_GetAttrString(pylearn2_datasets_processing, "ZCA");
                zca_obj = PyObject_CallObject(zca_fnp, NULL);
                Py_DECREF(zca_fnp);

                fit_name = PyString_FromString("fit");

                module = PyString_FromString("numpy");
                PyObject* numpy = PyImport_Import(module);
                Py_DECREF(module);

                numpy_dot_fnp = PyObject_GetAttrString(numpy, "dot");
                numpy_sub_fnp = PyObject_GetAttrString(numpy, "subtract");

                module = PyString_FromString("matplotlib.pyplot");
                PyObject* pyplot = PyImport_Import(module);
                Py_DECREF(module);

                display_fnp = PyObject_GetAttrString(pyplot, "imshow");
                display_do_fnp = PyObject_GetAttrString(pyplot, "savefig");
            }

            Mat flows[2]; 
            split(flow, flows);
            npy_arr_0 = converter.toNDArray(flows[0]);
            npy_arr_1 = converter.toNDArray(flows[1]);
            PyObject* orig_cvt_x = converter.toNDArray(flows[0]);
            PyObject* orig_cvt_y = converter.toNDArray(flows[1]); 

            PyObject_CallMethodObjArgs(zca_obj, fit_name, converter.toNDArray(flows[0]), NULL);

            PyObject* sub_result_0 = PyObject_CallFunctionObjArgs(numpy_sub_fnp, npy_arr_0, PyObject_GetAttrString(zca_obj, "mean_"), NULL);
            PyObject* result_0 = PyObject_CallFunctionObjArgs(numpy_dot_fnp, sub_result_0, PyObject_GetAttrString(zca_obj, "P_"), NULL);

            PyObject_CallMethodObjArgs(zca_obj, fit_name, converter.toNDArray(flows[1]), NULL);
            PyObject* sub_result_1 = PyObject_CallFunctionObjArgs(numpy_sub_fnp, npy_arr_1, PyObject_GetAttrString(zca_obj, "mean_"), NULL);
            PyObject* result_1 = PyObject_CallFunctionObjArgs(numpy_dot_fnp, sub_result_1, PyObject_GetAttrString(zca_obj, "P_"), NULL);
            //^ numpy.dot(X - self.mean_, self.P_)
            
            flows[0] = converter.toMat(result_0);
            flows[1] = converter.toMat(result_1);
