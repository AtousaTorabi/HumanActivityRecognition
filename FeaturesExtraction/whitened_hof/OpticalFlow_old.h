						 ...
						 std::vector<float>& fscales, int poly_n, double poly_sigma)
{
    ...
    for(int k = 0; k < poly_exp_pyr.size(); k++)
    {
        ...
        npy_arr = converter.toNDArray(fimg);
        PyObject* orig_cvt = converter.toNDArray(fimg);

        PyObject* correct = PyObject_CallMethodObjArgs(zca_obj, fit_name, npy_arr, NULL);
