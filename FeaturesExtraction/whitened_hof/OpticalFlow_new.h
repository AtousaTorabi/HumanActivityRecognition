                              ...
                              std::vector<Mat>& flow_pyr, int winsize, int iterations, bool warped=false)
{
    ...
    for( k = flow_pyr.size() - 1; k >= 0; k-- )
    {
        ...
        if(warped){
            ...
            npy_arr = converter.toNDArray(flow);
            PyObject* orig_cvt = converter.toNDArray(flow);

            PyObject_CallMethodObjArgs(zca_obj, PyString_FromString("fit"), npy_arr, NULL);

            PyObject* sub_result = PyObject_CallFunctionObjArgs(numpy_sub_fnp, npy_arr, PyObject_GetAttrString(zca_obj, "mean_"), NULL);

            fprintf(stderr, "mean_: %d", PyObject_HasAttrString(zca_obj, "mean_"));
            fprintf(stderr, "P_: %d", PyObject_HasAttrString(zca_obj, "P_"));

            PyObject* result = PyObject_CallFunctionObjArgs(numpy_dot_fnp, sub_result, PyObject_GetAttrString(zca_obj, "P_"), NULL);
            //^ numpy.dot(X - self.mean_, self.P_)
            
            fputs("hello", stderr);
            flow = converter.toMat(result);
            fputs("olleh", stderr);

            PyObject_CallFunctionObjArgs(display_fnp, result, NULL);
            PyObject* fname = PyString_FromString("output.png");
            PyObject_CallFunctionObjArgs(display_do_fnp, fname, NULL);
            Py_DECREF(fname);

            PyObject_CallFunctionObjArgs(display_fnp, orig_cvt, NULL);
            fname = PyString_FromString("output_orig.png");
            PyObject_CallFunctionObjArgs(display_do_fnp, fname, NULL);
            Py_DECREF(fname);
        }
