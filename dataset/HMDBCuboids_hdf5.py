...
class HMDBCuboidsDataset(dataset.Dataset):
    ...
    def __init__(self, data_path, split, which_set,
        ...
        print "INIT", data_path, split, which_set
        ...
        if which_set == 'train':
           ...
           self.data_path = data_path + '/%s_0/' % split
            ...
            self.data_path = data_path + '/%s_3/' % split


        print os.path.join(self.data_path, 'features.hdf')
        print os.path.join(self.data_path, 'files_lst.txt')
        self.read_ids(os.path.join(self.data_path, 'files_lst.txt'))

        ### Read data
        self.h5file = tables.openFile(os.path.join(self.data_path, 'features.hdf'),
                                      mode = "r")
        self.data = self.h5file.getNode('/', "denseFeat")
    ...
    def get_minibatch(self, firstIdx, lastIdx, batch_size,
        ...
        x = np.zeros([batch_size, self.data.shape[1]] , dtype="float32")
        y = np.zeros([lastIdx-firstIdx, self.nbTags],  dtype="float32")


        # Return a batch ('b', 0, 1, 't','c')
        print x[0:batch_size,:].shape, self.data[firstIdx:lastIdx, :].shape
    ...
    def iterator(self, mode=None, batch_size=None, num_batches=None, topo=None,
                 ...
                 shuffle_batch=True):
        return HMDBCuboidsIterator(self, batch_size, num_batches,
