...
class AtousaDataset(dataset.Dataset):
    ...
    def __init__(self, which_set, axes = ('b', 0, 1, 'c')):
        ...
        if self.which_set =='train':
            self.data_path = '/Tmp/zumerjer/UCF101/train1/'
            kind = 'train1'
            self.nbExamples = 9600
            file_n = self.data_path +'train1_idx.txt'
            self.Index=numpy.loadtxt(file_n)
            self.nbExamples = len(self.Index) - (len(self.Index) % 64)
            
        else:
            self.data_path = '/Tmp/zumerjer/UCF101/test1/'
            kind = 'test1'
            self.nbExamples = 3840
            file_n = self.data_path +'test1_idx.txt'
            self.Index=numpy.loadtxt(file_n)
            self.nbExamples = len(self.Index) - (len(self.Index) % 64)
            
        file_n2 = self.data_path + 'Index.txt'
        self.Interval = numpy.loadtxt(file_n2)
        file_n3 = self.data_path + kind + '_labels.txt'
    ...
    def get_minibatch(self, firstIdx, lastIdx, data_specs, return_tuple):
        ...
        for num1 in range(firstIdx, lastIdx):
            ...
            if s == 0: #XXX
                x[:,:,:,batchIdx] = numpy.zeros_like(x[:,:,:,batchIdx])
            elif s < 100:
                datatmp = data[start:end,:]
                t= int(math.ceil(100/s))
                dd=datatmp
                for k in range(0,t):
                     datatmp = numpy.append(datatmp,dd,axis=0)
                numpy.random.shuffle(datatmp)       
                R= random.randint(1,(len(datatmp)-100)) 
                datatmp=datatmp[R:R+100, 10:436]
            else:
                try:
                   R= random.randint(1,(s-100))
                   datatmp=data[start+R:start+R+100, 10:436]
                except:
                   import pdb
                   pdb.set_trace()
            #try:				   
            ...
            #     import pdb
            #     pdb.set_trace()
            y[batchIdx,:] = self.labels[num];
        #timer12 += time.time()
        self.h5file.close()
            
        # reshape the matrix 100X427 to 427X1X500
        #x = numpy.zeros(self.vidShape + [lastIdx-firstIdx,], dtype="float32")
        #y = numpy.zeros([lastIdx-firstIdx, self.nbTags],  dtype="float32")
