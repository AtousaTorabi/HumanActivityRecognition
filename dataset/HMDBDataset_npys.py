...
class HMDBnpysDataset(dataset.Dataset):
    ...
    def load_data(nb):
        assert nb < len(self.data_list)

        features_path = os.path.join(self.data_path, self.data_list[nb] + '_features.npy')
        labels_path = os.path.join(self.data_path, self.data_list[nb] + 'labels.txt')
        ids_path = os.path.join(self.data_path, self.data_list[nb] + 'files_lst.txt')

        print features_path
        print labels_path
        print ids_path

        self.read_ids(ids_path)
        ...
        self.nb_examples = self.data.shape[0] * len(self.data_list)
...
class HMDBfftIterator:
    ...
    def __init__(self, dataset=None, batch_size=None, num_batches=None,
        ...
        self._dataset_size = len(dataset.labels) * len(dataset.data_list)
    ...
    def next(self):
        if self._next_batch_no >= self._num_batches:
            npy_idx += 1
            if self.npy_idx >= len(self._dataset.data_list):
                raise StopIteration()
            else:
                self._next_batch_no = 0
                self._dataset.load_data(npy_idx)
        else:
